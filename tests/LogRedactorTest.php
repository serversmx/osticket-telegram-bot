<?php
/**
 * Tests for TgLogRedactor — PII redaction for log context arrays.
...
class LogRedactorTest {
    ...
    private function test_passthrough_non_array() {
        $this->assertSame('hello', TgLogRedactor::context('hello'), 'string passes through');
        $this->assertSame(42, TgLogRedactor::context(42), 'int passes through');
        $this->assertSame(null, TgLogRedactor::context(null), 'null passes through');
    }
    ...
    private function test_mask_phone_short() {
        $this->assertSame('****', TgLogRedactor::maskPhone('1234'), '4-digit phone fully masked');
        $this->assertSame('***',  TgLogRedactor::maskPhone('123'),  '3-digit phone fully masked');
    }

    private function test_mask_phone_normal() {
        $this->assertSame('********5678', TgLogRedactor::maskPhone('525512345678'), 'normal phone shows last 4');
    }

    private function test_mask_phone_array() {
        $in  = array('525511112222', '525533334444');
        $out = TgLogRedactor::maskPhone($in);
        $this->assertSame(array('********2222', '********4444'), $out, 'array of phones each masked');
    }

    private function test_preview_text_short() {
        $this->assertSame('[5 chars] hello', TgLogRedactor::previewText('hello'), 'short text shows length + full text');
    }

    private function test_preview_text_long() {
        $long = str_repeat('a', 100);
        $out  = TgLogRedactor::previewText($long);
    ...
    private function test_preview_text_collapses_whitespace() {
        $out = TgLogRedactor::previewText("line1\nline2\t\ttabs");
    ...
    private function test_preview_text_non_scalar() {
        $this->assertSame('[non-scalar]', TgLogRedactor::previewText(array('x')), 'array → [non-scalar]');
        $this->assertSame('[non-scalar]', TgLogRedactor::previewText(new stdClass()), 'object → [non-scalar]');
    }
